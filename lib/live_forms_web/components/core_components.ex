defmodule LiveFormsWeb.CoreComponents do
  @moduledoc """
  ...
  def flash(assigns) do
    ...
    <div
      ...
      role="alert"
      class={[
        "alert fixed block top-2 right-2 w-80 sm:w-96 z-50",
        @kind == :info && "alert-info",
        @kind == :error && "alert-error text-white"
      ]}
      {@rest}
    >
      <span :if={@title} class="flex items-center gap-1.5 text-[0.8125rem] font-semibold leading-6">
  ...
  def input(%{type: "textarea"} = assigns) do
    ...
    <div phx-feedback-for={@name}>
      ...
      <textarea
        ...
        class={[
          "textarea",
          @errors != [] && "border-error focus:border-error focus:ring-error/10"
        ]}
        {@rest}
  ...
  def translate_error({msg, opts}) do
    ...
    if count = opts[:count] do
      Gettext.dngettext(LiveFormsWeb.Gettext, "errors", msg, msg, count, opts)
    else
      Gettext.dgettext(LiveFormsWeb.Gettext, "errors", msg, opts)
    end
  end
